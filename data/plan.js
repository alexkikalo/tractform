/* Tractform — shared homestead plan + summary drawer
   Persists across pages in localStorage. Pages write slices; the drawer reads them.
*/
(function () {
  ...
  function load() {
    try {
      ...
      return {
        ...
        utilities: { ...base.utilities, ...(raw.utilities || {}) }
      };
    } catch {
      return emptyPlan();
    }
  ...
  function save(next) {
    const plan = { ...emptyPlan(), ...next, v: 1, updatedAt: Date.now() };
  ...
  function utilityWater(plan) {
    ...
    let gal = people * 75;
    const live = (plan.utilities && plan.utilities.livestockWater) || 'none';
    const irrig = (plan.utilities && plan.utilities.irrigation) || 'none';
    if (live === 'small') gal += 40;
    if (live === 'cattle') gal += 200;
    if (irrig === 'garden') gal += 50;
    if (irrig === 'field') gal += 300;
    const liveTot = livestockTotals(plan);
  ...
  function summarize(plan) {
    const p = plan || load();
    const loc = window.TractformLocation ? window.TractformLocation.get() : {};
    const landAcres = landAcresFromMix(p.land);
    const live = livestockTotals(p);
    const gardenSq = (p.gardens || []).reduce(function (s, g) { return s + (Number(g.area) || 0); }, 0);
    return {
      count: itemCount(p),
  ...
  function addLivestock(entry) {
    ...
    if (window.TractformLivestock && window.TractformLivestock.calculate) {
      const loc = window.TractformLocation ? window.TractformLocation.get() : {};
      const r = window.TractformLivestock.calculate({ animal, breed, quantity, zone: loc.zone || null });
      if (r) {
        extra = {
    ...
    const id = animal + ':' + breed;
    const next = plan.livestock.filter(function (row) { return row.id !== id; });
    next.push({
      id, animal, breed, quantity,
  ...
  function addBuilding(entry) {
    ...
    const sqft = Math.max(40, parseInt(entry.sqft, 10) || 0);
    const id = type + ':' + style;
    const next = plan.buildings.filter(function (row) { return row.id !== id; });
    next.push({
      id, type, style, sqft,
  ...
  function addGarden(entry) {
    ...
    const area = Math.max(50, parseInt(entry.area, 10) || 0);
    const water = entry.water || 'drip';
    const next = plan.gardens.filter(function (row) { return row.focus !== focus; });
    next.push({
      id: focus, focus, area, water,
      name: GARDEN_NAMES[focus] || focus
    });
  ...
  function toast(msg) {
    let el = document.getElementById('tf-plan-toast');
    if (!el) {
  ...
  function updateBadge() {
    const n = itemCount();
    const badge = document.getElementById('tf-plan-badge');
    if (!badge) return;
  ...
  function setOpen(open) {
    const drawer = document.getElementById('tf-plan-drawer');
    const overlay = document.getElementById('tf-plan-overlay');
    if (!drawer || !overlay) return;
    drawer.classList.toggle('translate-x-full', !open);
    overlay.classList.toggle('hidden', !open);
    overlay.classList.toggle('opacity-0', !open);
    document.body.classList.toggle('overflow-hidden', open);
    const btn = document.getElementById('tf-plan-btn');
  ...
  function lineRow(opts) {
    return (
      '<div class="flex items-start gap-2 py-2.5 border-b border-stone-100">' +
  ...
  function renderDrawer() {
    ...
    const people = document.getElementById('tf-plan-people');
    const acres = document.getElementById('tf-plan-acres');
    ...
    const placeEl = document.getElementById('tf-plan-place');
    if (placeEl) {
      const label = sum.place ? sum.place + (sum.zone ? ' · Z ' + sum.zone : '') : (sum.zone ? 'Zone ' + sum.zone : 'Set place in the header');
    ...
    if (!plan.land && !plan.livestock.length && !plan.buildings.length && !plan.gardens.length) {
      ...
      if (plan.land && plan.land.activities) {
        ...
        const bits = plan.land.activities.map(function (a) {
      ...
      if (plan.livestock.length) {
        ...
        plan.livestock.forEach(function (row) {
          html += lineRow({
            kind: 'livestock',
            id: row.id,
            title: row.quantity + ' × ' + (row.breedName || row.breed),
            meta: row.animalName || row.animal,
            right: row.landLabel || fmtAcres(row.landAcres)
          });
        });
      }
      if (plan.buildings.length) {
        ...
        plan.buildings.forEach(function (row) {
          html += lineRow({
            kind: 'building',
            id: row.id,
            title: row.typeName || BUILDING_NAMES[row.type] || row.type,
            meta: (row.styleName || row.style) + ' · ' + Number(row.sqft).toLocaleString() + ' sq ft'
          });
        });
      }
      if (plan.gardens.length) {
        ...
        plan.gardens.forEach(function (row) {
          html += lineRow({
            kind: 'garden',
            id: row.id,
            title: row.name || GARDEN_NAMES[row.focus] || row.focus,
            meta: Number(row.area).toLocaleString() + ' sq ft · ' + row.water
          });
        });
      }
    }
    ...
    const rec = document.getElementById('tf-plan-rec');
    const water = document.getElementById('tf-plan-water');
    const labor = document.getElementById('tf-plan-labor');
    const fit = document.getElementById('tf-plan-fit');
    if (rec) rec.textContent = sum.landAcres ? fmtAcres(sum.landAcres) : '—';
    if (water) water.textContent = sum.waterGal ? '~' + sum.waterGal + ' gal/day' : '—';
    if (labor) labor.textContent = laborLabel(sum.laborMin);
    if (fit) {
      if (sum.acresOnHand != null && sum.landAcres) {
        const delta = +(sum.acresOnHand - sum.landAcres).toFixed(2);
        if (delta >= 0) {
        ...
        const delta = +(sum.acresOnHand - sum.livestockLand).toFixed(2);
        fit.textContent = delta >= 0
          ? sum.acresOnHand + ' ac covers livestock land (+' + delta + ' ac). Add a land mix for the full lot.'
        ...
        fit.textContent = loc.state || loc.zone
          ? 'Add a land mix to see recommended lot size.'
          : 'Set a place, then add land or livestock.';
  ...
  function mountPlan() {
    if (document.getElementById('tf-plan-btn')) {
      updateBadge();
      return;
    }
    ...
    drawer.innerHTML =
      '<div class="h-14 px-4 border-b border-stone-200 flex items-center justify-between shrink-0">' +
        '<div>' +
          '<p class="text-sm font-semibold text-emerald-950">Your plan</p>' +
          '<p id="tf-plan-place" class="text-[11px] text-stone-500">Set place in the header</p>' +
        '</div>' +
        '<button type="button" id="tf-plan-close" class="w-8 h-8 rounded-lg text-stone-500 hover:bg-stone-100" aria-label="Close plan">×</button>' +
      '</div>' +
      '<div class="px-4 py-3 border-b border-stone-100 grid grid-cols-2 gap-2 shrink-0">' +
        '<label class="block"><span class="block text-[11px] text-stone-500 mb-1">People</span>' +
          '<input id="tf-plan-people" type="number" min="1" max="20" class="w-full rounded-lg border border-stone-300 px-2.5 py-1.5 text-sm" />' +
        '</label>' +
        '<label class="block"><span class="block text-[11px] text-stone-500 mb-1">Acres on hand</span>' +
          '<input id="tf-plan-acres" type="number" min="0" step="0.1" placeholder="Optional" class="w-full rounded-lg border border-stone-300 px-2.5 py-1.5 text-sm" />' +
        '</label>' +
      '</div>' +
      '<div id="tf-plan-body" class="flex-1 overflow-y-auto px-4 py-2"></div>' +
      '<div class="border-t border-stone-200 px-4 py-3 space-y-2 shrink-0 bg-stone-50">' +
        '<div class="flex justify-between text-sm"><span class="text-stone-500">Recommended lot</span><span id="tf-plan-rec" class="font-medium tabular-nums">—</span></div>' +
        '<div class="flex justify-between text-sm"><span class="text-stone-500">Peak water</span><span id="tf-plan-water" class="font-medium tabular-nums">—</span></div>' +
        '<div class="flex justify-between text-sm"><span class="text-stone-500">Chore time</span><span id="tf-plan-labor" class="font-medium tabular-nums">—</span></div>' +
        '<p id="tf-plan-fit" class="text-xs text-stone-500 leading-relaxed"></p>' +
        '<a href="/plan.html" class="block text-center rounded-lg bg-emerald-800 text-white text-sm font-medium py-2.5 hover:bg-emerald-900">View full plan</a>' +
      '</div>';
  ...
  function addButton(label) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'tf-add-plan inline-flex items-center justify-center rounded-lg bg-emerald-800 text-white text-sm font-medium px-4 py-2.5 hover:bg-emerald-900 transition';
    btn.textContent = label || 'Add to plan';
    return btn;
  }
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', mountPlan);
  else mountPlan();
})();
