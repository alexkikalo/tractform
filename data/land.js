...
(function () {
  ...
  const LOCAL_MARKETS = [
    ...
    },
    {
      id: 'tx-coppell-inner',
      state: 'TX',
      name: 'Inner-north DFW (Coppell / Grapevine / Carrollton)',
      match: [/\bcoppell\b/i, /\bgrapevine\b/i, /\bcarrollton\b/i, /\bfarmers branch\b/i, /\b75019\b/, /\b76051\b/, /\b75006\b/, /\b75007\b/, /\b75234\b/],
      homesteadLow: 80000,
      homesteadHigh: 200000,
      workingLow: 60000,
      workingHigh: 150000,
      note: 'In-town and inner-ring lots. True working acreage is scarce; prices track residential land more than farm averages.'
    }
  ];
  ...
  function fitParcel(haveAcres, calc) {
    const need = calc && calc.totalAcres != null ? Number(calc.totalAcres) : 0;
    const rows = (calc && calc.rows) || [];
    const place = (calc && calc.place) || '';
    const raw = haveAcres == null || haveAcres === '' ? NaN : Number(haveAcres);
    if (!Number.isFinite(raw) || raw < 0) {
      return {
        ...
        shortBy: null,
        ratio: null,
        headline: 'Enter acres on a parcel you have or are looking at.',
        detail: 'This checks whether the mix fits the land — not how much you should buy.',
        dominant: null,
        suggestion: null,
        place: place
      };
    }
    const have = +raw.toFixed(2);
    const spare = +(have - need).toFixed(2);
    const shortBy = spare < 0 ? +Math.abs(spare).toFixed(2) : 0;
    const ratio = need > 0 ? +(have / need).toFixed(2) : null;
    const where = place || 'this parcel';
    const sorted = rows.slice().sort(function (a, b) { return (b.acres || 0) - (a.acres || 0); });
    const dominant = sorted[0] || null;
    let status, headline, detail, suggestion = null;

    if (need <= 0) {
      status = 'unset';
      headline = have + ' ac on ' + where + '.';
      detail = 'Pick activities to test whether they fit.';
    } else if (spare >= Math.max(1, need * 0.15)) {
      status = 'fits';
      headline = have + ' ac in ' + where + ' covers this mix.';
      detail = spare.toFixed(spare >= 10 ? 0 : 1) + ' ac spare after the recommended ' + need + ' ac.';
    } else if (spare >= 0) {
      status = 'tight';
      headline = have + ' ac in ' + where + ' is a tight fit.';
      detail = 'Recommended mix wants ' + need + ' ac. Little room for lanes, septic setbacks, or a future barn.';
    } else {
      status = 'short';
      headline = have + ' ac in ' + where + ' does not fit this mix.';
      detail = 'Short ' + shortBy + ' ac versus ' + need + ' ac recommended.';
      if (dominant && dominant.acres >= shortBy) {
        const scales = dominant.scales || {};
        const smaller = Object.keys(scales).filter(function (k) {
          return scales[k].acres < dominant.acres;
        }).sort(function (a, b) { return scales[a].acres - scales[b].acres; });
        if (smaller.length) {
          const next = smaller[smaller.length - 1];
          const nextAc = scales[next].acres;
          const nextNeed = +(need - dominant.acres + nextAc).toFixed(2);
          suggestion = dominant.short + ' at ' + dominant.scaleLabel + ' wants ' + dominant.acres + ' ac by itself. Drop to ' + scales[next].label + ' (' + nextAc + ' ac) and the mix is ~' + nextNeed + ' ac.';
        } else {
          suggestion = dominant.short + ' at ' + dominant.scaleLabel + ' wants ' + dominant.acres + ' ac by itself. Drop that activity or find more land.';
        }
      } else if (dominant) {
        suggestion = dominant.short + ' is the largest slice (' + dominant.acres + ' ac). Scale it down or drop a use.';
      }
    }

    return { status: status, have: have, need: need, spare: spare, shortBy: shortBy, ratio: ratio, headline: headline, detail: detail, dominant: dominant, suggestion: suggestion, place: place };
  }
  ...
  function calculate(state) {
    ...
    result.fit = fitParcel(state.acresOnHand, result);
    return result;
  }
  ...
  window.TractformLand = {
    ...
    calculate: calculate,
    fitParcel: fitParcel,
    findLocalMarket: findLocalMarket,
    listingSearchUrls: listingSearchUrls
  };
})();
